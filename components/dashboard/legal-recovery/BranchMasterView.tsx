...
import { Search, PhoneCall, Filter, RefreshCw } from "lucide-react";

export default function BranchMasterView({ branchesList, banksList, loading, setShowMarketingForm }: { branchesList: any[], banksList: any[], loading: boolean, setShowMarketingForm: (state: any) => void }) {
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex items-center">
          <button 
            ...
            className={`px-4 py-4 h-full border border-[#E8E4DF] hover:bg-[#F5F0EA] rounded-xl text-[10px] font-semibold tracking-wider uppercase transition-all flex items-center gap-1.5 shadow-sm ${showFilterOptions || bankFilter ? 'bg-[#F5F0EA] text-[#1C1C1A]' : 'bg-[#FCFBF9] text-[#5D5B57]'}`}
          >
              ...
              return (
                <tr key={br.id} className="hover:bg-white transition-colors">
                  ...
                  <td className="py-3 px-4">
                    ...
                    <div className="text-[#9C9890] text-[10px] flex items-center gap-1 mt-0.5"><PhoneCall className="w-3 h-3"/> {br.branchManagerContact || 'N/A'}</div>
                    {br.branchEmail && <div className="text-[#9C9890] text-[10px] mt-0.5">{br.branchEmail}</div>}
                  </td>
                  <td className="py-3 px-4">
                    <div className="font-semibold text-slate-700">AO: {br.aoName || 'N/A'}</div>
                    <div className="text-slate-600">FO: {br.foName || 'N/A'}</div>
                    <div className="text-[#9C9890] text-[10px] flex items-center gap-1 mt-0.5"><PhoneCall className="w-3 h-3"/> {br.foContact || 'N/A'}</div>
                  </td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center justify-center">
                      ...
                      >
                        <PhoneCall className="w-3 h-3" /> Marketing Pitch
                      </button>
                    </div>
                  </td>
                </tr>
