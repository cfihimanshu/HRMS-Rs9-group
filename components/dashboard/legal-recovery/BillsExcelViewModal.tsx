...
import { INVOICE_COLLECTION_STATUSES } from "@/lib/invoice-collection";
...
export default function BillsExcelViewModal({ onClose, userRole, employees = [] }: { onClose: () => void; userRole?: string; employees?: any[] }) {
  ...
  const forwardInvoice = async () => {
    if (!forwardRow || !forwardEmployeeId || forwardSaving) return;
    setForwardSaving(true); setForwardError("");
    try {
      const response = await fetch("/api/legal-recovery/import-bills", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: forwardRow.id, action: "forward", employeeId: forwardEmployeeId }) });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || "Invoice forwarding failed");
      setRows(current => current.map(row => row.id === forwardRow.id ? { ...row, ...result.data } : row));
      setForwardRow(null);
    } catch (error: any) { setForwardError(error.message); }
    finally { setForwardSaving(false); }
  };
  const [collectionEdit, setCollectionEdit] = useState<{ invoice: any; status: string } | null>(null);
  ...
  const statusCounts = useMemo(() => {
    ...
    const matchingRows = rows.filter(row => (company === "All" || String(row.company) === company) && (poc === "All" || String(row.pocName || "Not Assigned") === poc) && (!q || [row.company, row.bankName, row.branchName, row.branchCode, row.invoiceNo, row.remark, row.pocName, row.collectionStatus, row.collectionRemark, row.collectionUpdatedByName].some(v => String(v || "").toLowerCase().includes(q))));
    return {
  ...
  const filtered = useMemo(() => rows.filter(row => {
    const q = query.toLowerCase();
    return (company === "All" || String(row.company) === company) && (poc === "All" || String(row.pocName || "Not Assigned") === poc) && (status === "All" || row.status === status) && (!q || [row.company, row.bankName, row.branchName, row.branchCode, row.invoiceNo, row.remark, row.pocName, row.collectionStatus, row.collectionRemark, row.collectionUpdatedByName].some(v => String(v || "").toLowerCase().includes(q)));
  ...
  const exportExcel = () => {
    const data = filtered.map(r => ({ Company: r.company, "Bill Date": date(r.billDate), "Invoice No.": r.invoiceNo, "Collection Status": r.collectionStatus || "", "Collection Remark": r.collectionRemark || "", "Collection Updated By": r.collectionUpdatedByName || "", "Collection Updated At": r.collectionUpdatedAt ? new Date(r.collectionUpdatedAt).toLocaleString("en-IN") : "", Bank: r.bankName, Branch: r.branchName, "Br. Code": r.branchCode, "POC Employee": r.pocName || "Not Assigned", "Bill Amount": Number(r.billAmount), "Pmt Rect. Date": date(r.paymentReceivedDate), "Receive Amount": Number(r.receivedAmount), TDS: Number(r.tdsAmount), "TDS %": Number(r.tdsPercent), Due: Number(r.dueAmount), Remark: r.remark, Status: r.status, "Revenue Type": r.revenueType, "Revenue Amount": Number(r.revenueAmount), "Internal Remark": r.internalRemark, "Assigned To": r.assignedTo }));
    const wb = XLSX.utils.book_new(); const ws = XLSX.utils.json_to_sheet(data); ws["!cols"] = [12,15,22,24,45,24,24,28,32,13,22,16,18,18,12,10,16,28,14,20,18,30,20].map(w => ({ wch: w })); XLSX.utils.book_append_sheet(wb, ws, "Bills"); XLSX.writeFile(wb, `Legal_Recovery_Bill_Register_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
  ...
  const columns = ["Company","Bill Date","Invoice No.","Collection Status","Collection Remark","Updated By","Updated At","Bank","Branch","Br. Code","POC Employee","Bill Amount","Pmt Rect. Date","Receive Amount","TDS","TDS %","Due","Remark","Status","Revenue Type","Revenue Amount","Internal Remark","Assigned To","Actions"];
