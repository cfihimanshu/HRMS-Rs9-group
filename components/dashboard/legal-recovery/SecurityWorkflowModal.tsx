...
export const SECURITY_WORKFLOW_STAGES = [
  ...
  { key: "site_closure", label: "Site Closure", hint: "Site close karte waqt final dues, relieved guards aur closure proof record karein." },
] as const;
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const stageMeta of SECURITY_WORKFLOW_STAGES) {
        ...
        if (stageMeta.key === "site_closure") {
          if (!stage.date || !stage.closureReason?.trim() || !stage.lastWorkingMonth || !stage.guardsRelievedDetails?.trim() || !stage.proofUrls.length) {
            return triggerToast("Site close karne ke liye closure date, reason, last working month, relieved guards aur closure proof required hain");
          }
          continue;
        }
    ...
    <div className="fixed inset-0 z-[100000] bg-slate-950/55 backdrop-blur-sm p-3 sm:p-6 flex items-center justify-center">
      <div className="bg-white w-full max-w-6xl max-h-[94vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <header className="px-5 py-4 border-b flex items-center justify-between bg-slate-50">
          <div><h2 className="font-black text-slate-900 flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-indigo-600"/> Security Work Pipeline</h2><p className="text-xs text-slate-500 mt-1">Bank visit se billing, payment aur site closure tak · {completed}/{SECURITY_WORKFLOW_STAGES.length} completed</p></div>
        ...
        <div className="grid lg:grid-cols-[330px_1fr] min-h-0 flex-1">
          ...
          <main className="p-5 overflow-y-auto">
            ...
            <section className="border rounded-2xl p-4">
              ...
              {activeKey !== "billing" && activeKey !== "payment_followup" && activeKey !== "site_closure" && <div className="grid sm:grid-cols-2 gap-3 mt-4">
              ...
              {activeKey !== "billing" && activeKey !== "payment_followup" && activeKey !== "site_closure" && <label className="block text-xs font-bold text-slate-600 mt-4">Discussion / Details<textarea value={active.notes} onChange={(e) => updateStage(activeKey, { notes: e.target.value })} rows={5} placeholder={meta.hint} className="mt-1 w-full border rounded-xl p-3 resize-y"/></label>}
