...
export default function BankWorkReportDashboard() {
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      if (!taskData.success) throw new Error(taskData.error || "Tasks load nahi hue");
      ...
      setError(err?.name === "AbortError" ? "Report load hone mein zyada samay lag raha hai. Retry karein." : err.message || "Bank work report load nahi hui");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) return <div className="min-h-[65vh] bg-white border rounded-2xl flex flex-col items-center justify-center p-6 text-center"><span className="p-3 bg-rose-50 text-rose-600 rounded-full"><AlertCircle className="w-7 h-7" /></span><h2 className="mt-3 font-black text-slate-800">Report load nahi hui</h2><p className="mt-1 text-xs text-slate-500">{error}</p><button type="button" onClick={loadReportData} className="mt-4 flex items-center gap-2 bg-[#714B67] text-white rounded-lg px-4 py-2 text-xs font-black"><RefreshCw className="w-4 h-4" /> Retry</button></div>;

  return <div className="space-y-4"><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><Building2 className="w-6 h-6 text-[#714B67]" /><h1 className="text-2xl font-black text-slate-900">Bank Work Dashboard</h1></div><p className="text-xs text-slate-500 mt-1">Recent task data ka bank aur branch-wise monitoring preview</p></div><button type="button" onClick={loadReportData} className="flex items-center gap-2 border bg-white hover:bg-slate-50 rounded-lg px-3 py-2 text-xs font-black text-slate-600"><RefreshCw className="w-4 h-4" /> Refresh</button></div><BankWorkReportModal open onClose={() => undefined} tasks={tasks} banks={banks} staff={staff} embedded /></div>;
}
