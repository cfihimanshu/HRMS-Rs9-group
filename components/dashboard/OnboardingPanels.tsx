...
export function OnboardingRoadmap({ selectedCandidate: initialCandidate, triggerToast }: OnboardingProps) {
  ...
  const [selectedCandidate, setSelectedCandidate] = useState<any>(initialCandidate || null);
  ...
  // Load all candidates for the sidebar
  const loadCandidates = async () => {
    try {
      ...
      if (data.success) {
        // Show all candidates, especially Shortlisted and Selected ones first
        ...
        // Auto-select candidate if none is selected
        if (!selectedCandidate && sorted.length > 0) {
          const defaultSelect = sorted[0];
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-3">
                    ...
                    <div className="h-2.5 bg-slate-200 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ 
                          width: `${
                            onboardingRecord.generatedDocs?.length 
...
export function TrainingClassroom({ triggerToast }: { triggerToast: (msg: string) => void; toggleModal?: any }) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      // Fetch trainees (Candidates who are Selected or in training phase)
      // Actually, we can fetch all active training pipelines from /api/trainings
      const res = await fetch("/api/trainings");
      const data = await res.json();
      
      // Also fetch candidates who are 'Selected' (i.e. finished onboarding) but don't have a training record yet
      const candRes = await fetch("/api/candidates");
      const candData = await candRes.json();

      if (data.success && candData.success) {
        const activeTrainings = data.data;
        const candidates = candData.data;

        // Merge: all active training records + candidates with status 'Selected' who don't have a training record
        const traineesList: any[] = [];
        
        // Add candidates from training records
        ...
        // Add 'Selected' candidates who aren't in training yet
        candidates.forEach((c: any) => {
          if (c.status === "Selected" && !activeTrainings.some((tr: any) => tr.candidate && tr.candidate._id === c._id)) {
            traineesList.push({ ...c, trainingRecord: null });
          }
        });

        setTrainees(traineesList);
        if (!selectedTrainee && traineesList.length > 0) {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="font-bold text-slate-800 text-xs truncate">{t.name}</div>
                      <span className="text-[10px] text-slate-500 truncate">{t.role || "Associate"}</span>
                    </div>
                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full border self-start ${
                      status === "Activation" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : 
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col h-[650px]">
              ...
              <div className="flex justify-between items-center gap-4 pb-4 border-b border-slate-150 shrink-0">
                ...
                <div className="px-3.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-right">
                  ...
                  <span className="text-xs font-bold text-[#714B67]">{trainingRecord ? trainingRecord.status : "Not Started"}</span>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-4">
                        ...
                        {trainingRecord.status !== "Activation" && trainingRecord.status !== "Final Status" && (
                          <form onSubmit={submitAssessment} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-4">
                              ...
                              <div className="space-y-3">
                                {[
                                  { label: "SOP Understanding", key: "sopScore" },
                                  { label: "CRM Understanding", key: "crmScore" },
                        ...
                        {(trainingRecord.status === "Final Status" || (trainingRecord.assessments?.length >= 3 && trainingRecord.status !== "Activation")) && (
                            ...
                            <div key={i} className="flex gap-4 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start">
                                  <span className="text-xs font-bold text-slate-800">Score: {a.marks}/100</span>
                                  <span className="text-[9px] text-slate-400">{new Date(a.date).toLocaleDateString()}</span>
                                </div>
                                <p className="text-[10px] text-slate-500 mt-1 font-semibold leading-relaxed">"{a.behaviourNotes}"</p>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full border self-start ${
                        status === "Confirm" ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : 
