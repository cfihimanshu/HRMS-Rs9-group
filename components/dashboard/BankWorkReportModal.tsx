  ...
  return <div className={embedded ? "w-full" : "fixed inset-0 z-[10000] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-3"} onMouseDown={event => { if (!embedded && event.target === event.currentTarget) onClose(); }}>
    <div className={embedded ? "bg-slate-50 rounded-2xl border w-full min-h-[calc(100vh-9rem)] overflow-hidden" : "bg-slate-50 rounded-2xl shadow-2xl w-full max-w-7xl max-h-[94vh] overflow-y-auto"}>
      <div className="sticky top-0 z-20 bg-white border-b px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3"><span className="p-2 rounded-xl bg-purple-50 text-[#714B67]"><Building2 className="w-5 h-5" /></span><div><h2 className="font-black text-slate-900">Bank Work Report</h2><p className="text-[10px] text-slate-500">Bank aur branch-wise task activity dashboard</p></div></div>
      ...
      <div className="p-5 space-y-4">
        ...
        {!bankName ? <div className="bg-white border border-dashed rounded-2xl py-20 text-center"><Building2 className="w-10 h-10 mx-auto text-slate-300 mb-3" /><h3 className="font-black text-slate-700">Bank select karein</h3><p className="text-xs text-slate-400 mt-1">Selected bank ki branch-wise work summary yahan dikhegi.</p></div> : <>
          ...
          <div className="bg-white border rounded-2xl overflow-hidden shadow-sm">
            ...
            {branchRows.length === 0 && <div className="py-16 text-center text-sm text-slate-400">Selected filters mein koi branch work record nahi mila.</div>}
          </div>
        </>}
      </div>
    </div>
