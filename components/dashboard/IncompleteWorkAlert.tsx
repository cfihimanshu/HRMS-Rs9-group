...
export default function IncompleteWorkAlert({ onOpenTasks }: { onOpenTasks: () => void }) {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/tasks/incomplete-alerts", { cache: "no-store" });
