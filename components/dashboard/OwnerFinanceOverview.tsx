...
export default function OwnerFinanceOverview({ onNavigate }: { onNavigate: (tab: string, filter?: string) => void }) {
  ...
  return <section className="rounded-2xl border-2 border-[#d8cbc3] bg-white shadow-sm overflow-hidden text-slate-900 [&_p]:font-extrabold [&_.text-slate-500]:text-slate-700 [&_.text-slate-600]:text-slate-800">
    ...
    {error ? <div className="p-5 text-xs text-rose-600 flex gap-2"><AlertTriangle className="w-4 h-4"/>{error}</div> : <>
      <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3 p-4">{summary.map(item => { const destination = item.name === "Legal Recovery" ? "masters" : undefined; return <div key={item.name} className="rounded-xl border border-[#ddd2ca] p-3 text-left hover:border-[#714B67]"><button type="button" onClick={() => onNavigate(item.tab, destination)} className="w-full flex items-center justify-between"><span className="flex items-center gap-2 text-[11px] font-black text-slate-900"><item.icon className="w-4 h-4 text-[#714B67]"/>{item.name}</span><ArrowUpRight className="w-3.5 h-3.5 text-slate-600"/></button><div className="grid grid-cols-3 gap-2 mt-3"><button type="button" onClick={() => onNavigate(item.tab, destination)} className="text-left rounded-md hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#714B67]/30"><p className="text-[9px] font-black text-slate-700">TOTAL</p><b className="text-sm text-slate-950 underline decoration-dotted underline-offset-2">{money(item.total)}</b></button><button type="button" onClick={() => onNavigate(item.tab, destination)} className="text-left rounded-md hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"><p className="text-[9px] font-black text-slate-700">RECEIVED</p><b className="text-sm text-emerald-700 underline decoration-dotted underline-offset-2">{money(item.received)}</b></button><button type="button" onClick={() => onNavigate(item.tab, destination)} className="text-left rounded-md hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-rose-500/30"><p className="text-[9px] font-black text-slate-700">REMAINING</p><b className="text-sm text-rose-700 underline decoration-dotted underline-offset-2">{money(item.pending)}</b></button></div></div>})}</div>
      <div className="grid xl:grid-cols-[0.8fr_1.2fr] border-t border-[#eee7e1]"><div className="p-4 xl:border-r border-[#eee7e1]"><h3 className="text-[11px] font-black text-[#321f2e] flex items-center gap-1"><Clock3 className="w-4 h-4"/> Pending Amount Age</h3><p className="text-[9px] text-slate-700 mt-1">Legal case remaining and Security bill pending amounts</p><div className="grid grid-cols-2 gap-2 mt-3">{aging.map(x => <button type="button" onClick={() => onNavigate("legal-recovery", "masters")} key={x.label} className={`rounded-lg border p-3 text-left hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#714B67]/30 ${x.style}`}><p className="text-[9px] font-black uppercase text-slate-800">{x.label}</p><b className="text-base font-black underline decoration-dotted underline-offset-2">{money(x.amount)}</b></button>)}</div><button onClick={() => onNavigate("vertical-dashboard")} className="mt-3 text-[10px] font-black text-[#5e3856]">Open complete finance dashboard <ArrowUpRight className="inline w-3.5 h-3.5"/></button></div>
      <div className="p-4"><h3 className="text-[11px] font-black text-[#321f2e] flex items-center gap-1"><IndianRupee className="w-4 h-4"/> Recently Received Money</h3><div className="mt-2 divide-y max-h-48 overflow-y-auto">{receipts.map(x => <button type="button" onClick={() => onNavigate("legal-recovery", "masters")} key={x.id} className="w-full py-2.5 grid grid-cols-[1fr_auto] gap-3 text-left hover:bg-slate-50"><div className="min-w-0"><p className="text-[10px] font-black text-slate-900 truncate">{x.client}</p><p className="text-[9px] font-extrabold text-slate-700">{x.vertical} · {x.by} · {when(x.date)}</p></div><b className="text-[12px] font-black text-emerald-700 underline decoration-dotted underline-offset-2">+{money(x.amount)}</b></button>)}{!receipts.length && <p className="py-6 text-center text-[10px] font-bold text-slate-700">No received payment records found.</p>}</div></div></div>
    </>}
  </section>;
}
